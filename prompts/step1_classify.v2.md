...
- `has_deadline` (true/false): true when the email creates a time-bound obligation for the recipient to act — i.e., the recipient has a concrete action that must happen by a specific time anchor (date, day-of-week, named event, or relative timeframe like "this week"). False when the email merely references urgency without a time anchor, asks ABOUT a deadline rather than stating one, or applies competitive pressure without a stated time for the recipient to act.
