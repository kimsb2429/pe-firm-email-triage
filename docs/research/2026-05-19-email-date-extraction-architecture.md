# Deep Dive: Simpler Architecture for Email Date Extraction
...
**Superhuman (LangChain co-published case study).** Their v1 pure-LLM agent explicitly "struggled to reason about dates accurately, e.g. identify upcoming deadlines." Their fix wasn't to bolt on Duckling — it was to restructure around (a) query classification, (b) tool calling with structured time-window arguments, (c) "double-dipping" critical instructions in both system and user messages. They added *structure around the LLM*, not a deterministic parser before it.
...
**Lindy, Cal.ai, Fyxer, Motion, etc.** — no public engineering disclosures. Inference is they're LLM-first with tool calls into a typed API, not parser-first.
...
### 4. Recognizers-Text's `Mod` field is the unsung hero for deterministic deadline semantics
...
**Gaps that remain even with Recognizers-Text:** business abbreviations (EOW/EOD/COB/EOM) — none of the three TIMEX libraries handles these natively. A ~20-line regex preprocessor that maps "by EOW" → "by end of this week", "by COB Friday" → "by end of day Friday" closes the gap.

**Python port caveat:** Recognizers-Text's Python port lags the .NET/JS ports on quarter expressions specifically. Worth a 10-minute spike to verify "Q1 board pack" parses cleanly in the Python version before committing.
...
That's the whole thing. No separate dateparser call, no custom regex, no ambiguity layer — the LLM handles all of it given the anchor + few-shot. `max_retries=2` covers Pydantic validation failures. Add a server-side `day_of_week(resolved_iso) == claimed_dow` validator if you want the off-by-one self-check.
