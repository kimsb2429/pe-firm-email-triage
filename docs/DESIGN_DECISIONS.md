...
### Two LLM calls per email, not one, not four
...
**Chose:** Two calls.
**Why:** (a) collapses the brief's framing that each step depends on the prior one into a single prompt — works fine but makes the dependency invisible. (c) ballooned to 4 schemas, 4 prompts, 4 call sites for what's logically one decision. Two calls preserve the sequential phases without sprawl.
...
**Trade-off:** Step 1 has to emit signal fields (`has_deadline`, `portco_problem_flagged`) so Python can decide. Initially resisted adding these — reversed when the routing payoff became clear.
...
**Why:** Classification is bounded and Haiku is fine at it. Reply quality is where the brief puts its evaluation weight, so Sonnet earns its cost there. Deadline extraction is bounded — Haiku is enough.
**Trade-off:** Two model strings instead of one. ~5 lines of code more.

### Step 2 sees raw email + Step 1 output, not just the summary
...
**Why:** Replies and next steps must reference specific names, numbers, deadlines — losing those to a summary guarantees generic outputs, which the brief explicitly evaluates against.
...
### `output_config.format` json_schema, not tool use
...
**Why:** Anthropic's native JSON outputs is the semantically honest mechanism — we want structured data, not a function call. Tool use was the historical workaround; JSON outputs is now GA. Less ceremony, same grammar-constrained reliability.
**Trade-off:** None at runtime. Schema must satisfy Anthropic's strict-mode requirements (`additionalProperties: false`, every property in `required`) — handled by `_strictify_schema()` helper.
...
**Why:** Grammar-constrained sampling emits fields in schema order. Putting `rationale` first forces the model to articulate reasoning *before* committing to decisions — improves output quality, gives a debug trace.
**Trade-off:** Extra tokens. Honest framing: rationale is a justification, not a faithful trace — use for debuggability, not interpretability claims.
...
**Why:** Without signals, we can't skip Step 2 for any email — Step 2 always has to run to detect triggers. With signals, Step 1 (cheap Haiku) tells us whether Step 2 (expensive Sonnet) is needed. Direct cost saving.
**Trade-off:** Step 1 carries more decisions than the spec literally lists. The spec describes outputs, not internal signals — defensible.

---

## Prompts

### Externalize prompts to `.md` files, not Python string literals
...
**Why:** Initial implementation defeated prompt caching — each system prompt was unique because the email body was embedded. Splitting fixed it: 0 cache reads → 9,603 cache reads on the same dataset.
...
**Why:** At 20 emails, distributed tracing is theatre. The five lean pieces cover every real failure mode (per-call audit, per-decision explanation, per-run summary, error capture, sanity counts).
**Trade-off:** No central dashboard. Add one when volume justifies it.

### One retry on `ValidationError`, not unconditional retries
...
**Why:** If strict mode + one retry still fails, the prompt is broken — looping won't fix it. Anthropic SDK already handles transient HTTP retries; ours covers schema drift.
...
**Why:** Layers 1–4 are free, catch real bugs (especially layer 3 — agent says "High priority" but didn't draft a reply). Layers 5–6 are heuristic, misfire-prone, and better handled by the grader subagent at dev time.
